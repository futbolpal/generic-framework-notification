...
public class TrayNotification extends JWindow implements ActionListener, Module {
	...
	private TrayNotification() {
		...
		time_in_ = 0;
		main_ = new JPanel();
		main_.setBorder(new CompoundBorder(new BevelBorder(BevelBorder.RAISED),
		...
		this.setLocation(SCREEN_WIDTH - this.getWidth() - insets.right,
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (this_.getLocation().getY() != destination && !abort_) {
					...
					this_.setLocation((int) this_.getLocation().getX(),
							(int) this_.getLocation().getY() + direction);
					try {
						Thread.sleep(10);
					} catch (Exception e) {
	 ...
	 * @param p - The
	 * component p contains the gui layout to be
	...
	/**
	 * Do nothing on close, so return true to
	 * indicate we closed successfully
	 */
	...
	/**
	 * 
	 * @return TrayNotification handle
	 */
